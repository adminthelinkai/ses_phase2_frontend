...
import { getProject, updateProject, UpdateProjectPayload } from '../../api/projects';
...
type TabType = 'project' | 'documents';

const EditProjectModal: React.FC<EditProjectModalProps> = memo(({ projectId, onClose, onSuccess }) => {
    ...
    const fetchProjectData = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          // Convert ISO8601 dates to date input format (YYYY-MM-DD)
          ...
          setErrors({ general: result.error || 'Failed to load project data' });
          showToast(result.error || 'Failed to load project data', 'error');
        }
      } catch (error) {
  ...
  }, [formData, projectId, validateForm, showToast, onSuccess, onClose]);

  // Handle backdrop click
  const handleBackdropClick = useCallback((e: React.MouseEvent) => {
    if (e.target === e.currentTarget && !isSubmitting && !isLoadingProject) {
      onClose();
    }
  }, [onClose, isSubmitting, isLoadingProject]);
  ...
  if (isLoadingProject) {
    return (
      ...
      >
        ...
        <div className="relative w-full max-w-md bg-[var(--bg-sidebar)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden">
          <div className="p-12 flex flex-col items-center justify-center">
            <div className="w-8 h-8 border-2 border-[var(--accent-blue)] border-t-transparent rounded-full animate-spin mb-4" />
            <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wide">Loading project data...</p>
          </div>
        </div>
      </div>
    );
      ...
      {!showUploadModal && (
        <div className="relative w-full max-w-2xl bg-[var(--bg-sidebar)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="px-6 py-4 border-b border-[var(--border-color)] flex items-center justify-between">
            <div>
              <h2 className="text-[13px] font-black text-[var(--text-primary)] uppercase tracking-tight">Edit Project</h2>
          ...
          {/* Tabs */}
          <div className="flex border-b border-[var(--border-color)] bg-[var(--bg-panel)]/40">
            <button
              onClick={() => setActiveTab('project')}
              className={`flex-1 px-4 py-3 text-[9px] font-black uppercase tracking-widest transition-all ${
                activeTab === 'project'
                  ? 'text-[var(--accent-blue)] border-b-2 border-[var(--accent-blue)] bg-[var(--bg-sidebar)]'
                  : 'text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-sidebar)]/50'
              }`}
            >
              Project Details
            </button>
            ...
            {activeTab === 'project' ? (
              ...
              <div className="p-6">
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wide mb-4">
                    Document Management
                  </p>
                  <button
                    onClick={() => setShowUploadModal(true)}
                    disabled={isSubmitting}
                    className="px-6 py-3 bg-[var(--accent-blue)] rounded-xl text-[9px] font-black uppercase tracking-widest text-white hover:bg-[var(--accent-blue)]/90 transition-all disabled:opacity-50"
                  >
                    Upload Document
                  </button>
                </div>
